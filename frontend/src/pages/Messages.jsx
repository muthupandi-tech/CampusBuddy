  ...
  return (
    <DashboardLayout>
      <div className="flex h-[calc(100vh-140px)] card overflow-hidden">
            ...
            {loadingContacts ? (
              ...
              contacts.map(contact => (
                <div 
                  ...
                  className={`p-4 cursor-pointer flex items-center space-x-3 transition-colors border-b border-slate-50 ${
                    activeContact?.id === contact.id ? 'bg-brand-50' : 'hover:bg-slate-50'
                  }`}
                >
                  <div className="h-10 w-10 rounded-full bg-slate-200 flex items-center justify-center flex-shrink-0 text-slate-600 font-bold">
                    {contact.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-medium text-slate-900 truncate">{contact.name}</h3>
        ...
        {/* Chat Area */}
        {activeContact ? (
          <div className={`flex-1 flex flex-col ${activeContact ? 'flex' : 'hidden md:flex'}`}>
            <div className="h-16 border-b border-slate-200 flex items-center px-6 bg-white shrink-0">
              <button 
                onClick={() => setActiveContact(null)} 
                className="md:hidden mr-4 text-brand-600 font-medium text-sm"
              >
                &larr; Back
              </button>
              <div className="flex items-center space-x-3">
                 <div className="h-8 w-8 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 font-bold shadow-sm">
                ...
                return (
                  <div key={msg.id || index} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] rounded-2xl px-4 py-2 shadow-sm relative ${
                      isMine ? 'bg-brand-600 text-white rounded-br-sm' : 'bg-white text-slate-800 border border-slate-100 rounded-bl-sm'
                    }`}>
                      <p className="text-sm break-words leading-relaxed">{msg.message}</p>
                      <span className={`text-[10px] mt-1 block right-0 ${isMine ? 'text-brand-200' : 'text-slate-400'}`}>
            ...
            <div className="p-4 bg-white border-t border-slate-200 shrink-0">
              <form onSubmit={handleSendMessage} className="flex space-x-2">
                <input 
                  ...
                  className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-full focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all text-sm"
                />
                <button 
                  type="submit" 
                  disabled={!newMessage.trim()}
                  className="h-12 w-12 rounded-full bg-brand-600 text-white flex items-center justify-center hover:bg-brand-700 transition-colors disabled:opacity-50 shadow-md"
                >
          ...
          <div className="hidden md:flex flex-1 items-center justify-center bg-slate-50">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-slate-800">Your Messages</h3>
              <p className="text-slate-500 text-sm mt-1">Select a contact to start messaging</p>
            </div>
          </div>
        )}
      </div>
