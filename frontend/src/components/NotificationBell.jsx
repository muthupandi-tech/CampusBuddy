    ...
    <div className="relative">
      ...
      {showDropdown && (
        <>
          ...
          ></div>
          <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden animate-fade-in-up">
            ...
            <div className="max-h-[400px] overflow-y-auto">
              {notifications.length === 0 ? (
                ...
                notifications.map(n => (
                  <div 
                    key={n.id} 
                    className={`p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors ${!n.is_read ? 'bg-brand-50/30' : ''}`}
                    onClick={() => !n.is_read && handleMarkRead(n.id)}
                  >
                    <div className="flex gap-3">
                      <div className={`mt-1 h-2 w-2 rounded-full shrink-0 ${!n.is_read ? 'bg-brand-500' : 'bg-transparent'}`}></div>
                      <div className="flex-1">
                        <p className={`text-sm ${!n.is_read ? 'text-slate-800 font-semibold' : 'text-slate-600'}`}>
                          {n.message}
                        </p>
